...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const logoImg: string = files.image![0].path;
...
router.get("/store-products/:store_id", async (req: Request, res: Response) => {
  try {
...
router.get("/single-product/:store_id/:product_id", async (req: Request, res: Response) => {
  try {
...
router.post("/add-printfull-product-to-cart/:store_id/:product_id", async(req: Request, res: Response) => {
  try {
    ...
    const addToUsersCart = await addToprintfullCart(payload)
...
router.get("/store/:storeId/wallet" , async(req:Request,res:Response)=>{
  try{
    console.log("searching the seller's wallet address")
      const { storeId } = req.params; ; 
      const seller = await getStoreWalletAddress(Number(storeId)); 
      console.log(seller)
      res.status(200).json({msg:"wallet address found" , walletAddress:seller?.walletAddress})
  }catch(err){
