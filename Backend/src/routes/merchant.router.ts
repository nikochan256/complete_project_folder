...
const generateUniqueSellerId = async (): Promise<number> => {
  let attempts = 0;
  ...
  throw new Error(
    "Failed to generate unique seller ID after multiple attempts"
  );
};
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const uniqueId = await generateUniqueSellerId();
      ...
      const kybDocumentUrl = await uploadToCloudinary(
        files.kybDocument[0].buffer,
        "kyb-documents",
        "raw",
        `kyb_${body.businessEmail
          .replace("@", "_")
...
router.put("/Merchat_update-password", async (req, res) => {
  try {
...
router.post("/add-recently-viewed", async (req: Request, res: Response) => {
  try {
    const {
      userId,
      productId,
    ...
    const existingView = await prisma.recentlyViewedProduct.findFirst({
      where: {
    ...
    if (existingView) {
      ...
      const updated = await prisma.recentlyViewedProduct.update({
    ...
    const recentlyViewed = await prisma.recentlyViewedProduct.create({
      data: {
        userId: parseInt(userId),
        productId: parseInt(productId),
        storeId: parseInt(storeId),
    ...
    const allViewed = await prisma.recentlyViewedProduct.findMany({
...
// Add this route to your user routes file
router.get("/recently-viewed/:userId", async (req: Request, res: Response) => {
  try {
    ...
    const recentlyViewed = await prisma.recentlyViewedProduct.findMany({
      where: { userId: parseInt(userId) },
      orderBy: { viewedAt: "desc" },
      take: limit,
    });
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log("New images count:", files?.length || 0);
      ...
      if (files && files.length > 0) {
        const imageUrls = await Promise.all(
          files.map((file) => uploadToCloudinary(file.buffer, "product-images"))
        );
      ...
      if (body.deleteImage2 === "true") updateData.image2 = null;
      if (body.deleteImage3 === "true") updateData.image3 = null;
...
router.get("/products/:sellerId", async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `✅ Fetched ${products.length} products for seller ${sellerId}`
    );
...
router.get("/all-merchants-products", async (req: Request, res: Response) => {
  try {
    ...
    const totalProducts = merchantsWithProducts.reduce(
      (sum, merchant) => sum + merchant.productsCount,
      0
    );
    const merchantsWithProducts_count = merchantsWithProducts.filter(
      (m) => m.productsCount > 0
    ).length;

    console.log(
      `✅ Fetched ${totalMerchants} merchants with ${totalProducts} total products`
    );
...
router.post("/verify-printful-api", async (req, res) => {
  try {
...
router.get("/get-all-stores", async (req: Request, res: Response) => {
  try {
    const all_stores = await getAllStores();
    res
      .status(200)
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    if (status && typeof status === "string") {
      ...
      merchants = await getMerchantsByStatus(upperStatus as KYBStatus);
    } else {
...
router.post(
  "/create-product",
  upload.array("images", 3), // Accept up to 3 product images
  async (req: Request, res: Response) => {
    try {
      const body = req.body;
      const files = req.files as Express.Multer.File[];
      console.log("this is the body received from the frontend", body);
      console.log("Number of images received:", files?.length || 0);
...
router.get("/seller-info/:sellerId", async (req: Request, res: Response) => {
  try {
    const sellerId = parseInt(req.params.sellerId);
...
router.put("/update-seller/:sellerId", async (req: Request, res: Response) => {
  try {
    const sellerId = parseInt(req.params.sellerId);
...
router.get("/all-orders/:sellerId", async (req: Request, res: Response) => {
  try {
    const sellerId = parseInt(req.params.sellerId);
    ...
    const formattedOrders = orderItems.map((orderItem) => ({
      ...
      totalAmount:
        orderItem.totalAmount || orderItem.productPrice * orderItem.quantity,
...
// 2. Get Recent Orders
router.get("/recent-orders/:sellerId", async (req: Request, res: Response) => {
  try {
    const sellerId = parseInt(req.params.sellerId);
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const products = await fetch("https://api.printful.com/store/products", {
        headers: {
          authorization: `Bearer ${api_key}`,
...
router.get(
  "/single-product/:store_id/:product_id",
  async (req: Request, res: Response) => {
    try {
      console.log("request reached here");
      const store_id = req.params.store_id;
      ...
      const store = await getstoreApiKey(Number(store_id));
...
router.delete("/cart-item/:cartItemId", async (req: Request, res: Response) => {
  try {
    ...
    const deletedFromCartItem = await deleteCartItemFromCart(payload);
...
router.patch("/cart-item/:cartItemId", async (req: Request, res: Response) => {
  try {
    ...
    const updatedCart = await updateQuantityinCart(payload);
...
router.get(
  "/product/:productId/seller-wallet",
  async (req: Request, res: Response) => {
    try {
...
router.patch("/order-item/:orderItemId/status", async (req, res) => {
  ...
  const updatedItem = await updateOrderStatus(payload);

  res.json({ success: true, orderItem: updatedItem });
});

router.get("/store_api_key/:storeId", async (req: Request, res: Response) => {
  try {
    const storeId = Number(req.params.storeId);
    const store = await getstoreApiKey(storeId);
    res.status(200).json({ apiKey: store?.api_key });
  } catch (err) {
    res.status(500).json({ msg: err });
  }
});
